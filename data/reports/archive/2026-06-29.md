...
1. 选一个与你当前工作流最接近的场景，围绕「小红书 RedKnot 推理引擎：将 KV Cache 按注意力头拆解实现长文本加速」写下 1 个可验证的小假设。
...
1. **美军用AI选目标却误炸伊朗学校，Anthropic Claude嵌入Palantir系统首日建议约1000目标** — The Decoder：AI News（RSS）（行业与平台）
...
2. **小红书 RedKnot 推理引擎：将 KV Cache 按注意力头拆解实现长文本加速** — 公众号：小红书技术（dots.llm）（产品与应用）
   2026-06-29 19:00 CST
...
3. **EverOS：开源Markdown优先智能体记忆运行时，支持混合检索与自进化技能** — MarkTechPost（RSS）（产品与应用）
   2026-06-29 18:42 CST
...
4. **Claude Code 打开 GitHub 仓库即执行隐藏恶意代码，攻击者可获完全控制** — The Decoder：AI News（RSS）（论文研究）
   2026-06-29 18:04 CST
...
5. **三星和SK海力士计划投资5900亿美元扩产芯片，AI需求推高内存价格** — The Decoder：AI News（RSS）（行业与平台）
   2026-06-29 16:17 CST
...
6. **OpenAI 报告：绘制欧洲 AI 劳动力机遇版图** — OpenAI：官网动态（RSS · 排除企业/客户案例）（论文研究）
   2026-06-29 15:00 CST
   OpenAI 发布新报告，分析 AI 对欧盟就业的影响，划定哪些职业面临自动化、增长或工作流程变化。
   https://openai.com/index/mapping-ai-jobs-transition-eu

7. **分享两个Vibe Coding必备的实用Prompt：第一性原理与对抗式审查** — 公众号：数字生命卡兹克（技巧与观点）
   2026-06-29 10:08 CST
...
8. **Artifacts 22：Zyphra、Cohere 和 Poolside 正在扩展生态系统广度** — Nathan Lambert：Interconnects（RSS）（技巧与观点）
   2026-06-29 01:03 CST
...
9. **Wayfinder Router：在本地和托管的大语言模型之间进行确定性查询路由** — Hacker News 热门（buzzing.cc 中文翻译）（产品与应用）
   2026-06-29 00:58 CST
